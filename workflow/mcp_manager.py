...
class MCPServerRegistry:
    ...
    def __init__(self):
        logger.info("[MCP Registry] Initializing MCP server registry")
    ...
    def list_servers(self) -> List[MCPServerConfig]:
        ...
        logger.info(f"[MCP Registry] Listing servers: count={len(servers_data)}")
    ...
    def get_server(self, server_id: str) -> Optional[MCPServerConfig]:
        ...
        for server_data in self._load_servers():
            if server_data.get("id") == server_id:
                logger.info(f"[MCP Registry] Found server: id={server_id}, name={server_data.get('name')}")
                return MCPServerConfig(**server_data)
        logger.info(f"[MCP Registry] Server not found: id={server_id}")
        return None
    ...
    def add_server(self, config: MCPServerConfig) -> MCPServerConfig:
        ...
        for s in servers:
            if s.get("name") == config.name:
                logger.info(f"[MCP Registry] Add server failed: duplicate name '{config.name}'")
                raise ValueError(f"Server with name '{config.name}' already exists")

        servers.append(config.model_dump())
        self._save_servers(servers)
        logger.info(f"[MCP Registry] Server added: id={config.id}, name={config.name}, command={config.command}")
        return config

    def update_server(self, server_id: str, **updates) -> Optional[MCPServerConfig]:
        """Update an existing server config."""
        servers = self._load_servers()
        logger.info(f"[MCP Registry] Updating server: id={server_id}, updates={list(updates.keys())}")
        ...
        for i, s in enumerate(servers):
            if s.get("id") == server_id:
                ...
                if new_name and new_name != s.get("name"):
                    for other in servers:
                        if other.get("id") != server_id and other.get("name") == new_name:
                            logger.info(f"[MCP Registry] Update failed: duplicate name '{new_name}'")
                ...
                servers[i] = s
                self._save_servers(servers)
                logger.info(f"[MCP Registry] Server updated: id={server_id}, name={s.get('name')}")
                return MCPServerConfig(**s)

        logger.info(f"[MCP Registry] Update failed: server not found id={server_id}")
        return None
    ...
    def delete_server(self, server_id: str) -> bool:
        ...
        if len(servers) < original_count:
            self._save_servers(servers)
            logger.info(f"[MCP Registry] Server deleted: id={server_id}")
            return True
        logger.info(f"[MCP Registry] Delete failed: server not found id={server_id}")
        return False
...
class MCPProcessManager:
    ...
    def __init__(self, registry: MCPServerRegistry):
        ...
        logger.info("[MCP ProcessManager] Initialized process manager")
    ...
    def start_server(self, server_id: str) -> bool:
        ...
        """
        logger.info(f"[MCP ProcessManager] Starting server: id={server_id}")

        # Check if already running
        if self.get_status(server_id) == MCPServerStatus.RUNNING:
            logger.info(f"[MCP ProcessManager] Server already running: id={server_id}")
            return True
        ...
        if config is None:
            ...
            logger.info(f"[MCP ProcessManager] Start failed: config not found for id={server_id}")
            return False
        ...
        try:
            ...
            logger.info(f"[MCP ProcessManager] Executing command: {' '.join(cmd)}")

            # Prepare environment
            env = os.environ.copy()
            env.update(config.env)
            if config.env:
                logger.info(f"[MCP ProcessManager] Custom env vars: {list(config.env.keys())}")
            ...
            logger.info(f"[MCP ProcessManager] Server started: id={server_id}, pid={proc.pid}, name={config.name}")
            return True

        except FileNotFoundError as e:
            self._error_messages[server_id] = f"Command not found: {config.command}"
            logger.info(f"[MCP ProcessManager] Start failed: command not found '{config.command}'")
            return False
        except PermissionError as e:
            self._error_messages[server_id] = f"Permission denied: {config.command}"
            logger.info(f"[MCP ProcessManager] Start failed: permission denied '{config.command}'")
            return False
        except Exception as e:
            self._error_messages[server_id] = f"Failed to start: {str(e)}"
            logger.info(f"[MCP ProcessManager] Start failed: {str(e)}")
            return False
    ...
    def stop_server(self, server_id: str) -> bool:
        ...
        """
        logger.info(f"[MCP ProcessManager] Stopping server: id={server_id}")

        proc = self._processes.get(server_id)
        if proc is None:
            logger.info(f"[MCP ProcessManager] Server already stopped: id={server_id}")
            return True

        pid = proc.pid
        try:
            # Try graceful termination first
            logger.info(f"[MCP ProcessManager] Sending SIGTERM to pid={pid}")
            if os.name == 'nt':
                # Windows
                proc.terminate()
            else:
            ...
            try:
                ...
                logger.info(f"[MCP ProcessManager] Server stopped gracefully: id={server_id}, pid={pid}")
            except subprocess.TimeoutExpired:
                # Force kill if graceful termination failed
                logger.info(f"[MCP ProcessManager] Timeout waiting, sending SIGKILL to pid={pid}")
                if os.name == 'nt':
                    proc.kill()
                else:
                    try:
                ...
                logger.info(f"[MCP ProcessManager] Server force killed: id={server_id}, pid={pid}")
            ...
            logger.info(f"[MCP ProcessManager] Stop failed: id={server_id}, error={str(e)}")
            return False
    ...
    def restart_server(self, server_id: str) -> bool:
        ...
        """
        logger.info(f"[MCP ProcessManager] Restarting server: id={server_id}")
    ...
    def stop_all(self) -> int:
        ...
        """
        logger.info(f"[MCP ProcessManager] Stopping all servers: count={len(self._processes)}")
        count = 0
        for server_id in list(self._processes.keys()):
            if self.stop_server(server_id):
                count += 1
        logger.info(f"[MCP ProcessManager] Stopped {count} servers")
        return count
    ...
    def get_running_servers(self) -> List[MCPServerInfo]:
        ...
        for config in self._registry.list_servers():
            if self.get_status(config.id) == MCPServerStatus.RUNNING:
                ...
                if info:
                    result.append(info)
        logger.info(f"[MCP ProcessManager] Running servers: count={len(result)}, names={[s.name for s in result]}")
        return result
...
def init_mcp_manager() -> None:
    ...
    for server in auto_start_servers:
        logger.info(f"[MCP] Auto-starting server: name={server.name}, id={server.id}")
        if pm.start_server(server.id):
            started_count += 1
        else:
            logger.info(f"[MCP] Failed to auto-start server: name={server.name}")

    logger.info(f"[MCP] MCP manager initialized, auto-started {started_count}/{len(auto_start_servers)} servers")
...
def shutdown_mcp_manager() -> int:
    ...
    if _process_manager is not None:
        ...
        logger.info(f"[MCP] MCP manager shutdown complete, stopped {count} servers")
        return count
    logger.info("[MCP] MCP manager shutdown: no process manager active")
    return 0
